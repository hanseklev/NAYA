...
export const addFirstProduct = product => {
  ...
  localStorage.setItem("naya-cart", JSON.stringify(newCart))
...
export const removeItemFromCart = productId => {
  ...
  let existingCart = localStorage.getItem("naya-cart")
  existingCart = JSON.parse(existingCart)

  // If there is only one item in the cart, delete the cart.
  if (1 === existingCart.products.length) {
    localStorage.removeItem("naya-cart")
    return null
  }
...
/* Returns formatted cart error */

export function getShippingPrice(shipping) {
  let getFloatPrice = getFloatVal(shipping)
  if (getFloatPrice === 0) getFloatPrice = 49
  return getFloatPrice
}

export function getTotalPriceWithShipping(cart) {
  let float = getFloatVal(cart.subtotal)
  return float + getShippingPrice(cart.shippingTotal)
}

export function getCartError(err) {
  const formattedError = removeHTMLTags(err.graphQLErrors[0]?.message)
  if (formattedError === "")
    return "Noe gikk galt..Prøv å oppdater siden og prøv igjen"
